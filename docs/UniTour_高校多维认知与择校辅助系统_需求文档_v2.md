...
14. 统一返回结果
15. 全局异常处理
16. Docker Compose 管理 MySQL 和 Redis
17. README 与接口说明
...
- 用户名唯一
- 密码不建议明文入库，推荐 BCrypt
- 注册阶段只收集账号必要信息，降低用户进入门槛
- 注册成功返回统一结果

#### 用户资料完善

注册后不强制立即填写完整资料。用户可以先浏览高校、校区、POI、评分和评论区。
...
- `profileCompleted` 是下一阶段资料完善模块的设计项，当前基础认证模块尚未实现该字段
- 实现方式待定：可以在 `users` 表新增 `profile_completed` 字段，也可以根据 `identityType`、`targetUniId`、`currentUniId` 等字段动态判断
- 当前倾向：新增 `profile_completed` 字段，便于发布评价、点赞、回复等贡献型行为做权限判断
- 资料完善逻辑由后端校验，不能只依赖前端提示
...
- 从 JWT 中获取 userId
- 返回当前用户基本信息
- 资料完善模块完成后，应返回 `profileCompleted`，用于前端判断是否允许参与互动

#### 行为权限规则

UniTour 将用户行为分为浏览型和贡献型：

- 未登录用户：可以浏览高校列表、高校详情、校区 POI、评分汇总和评论区
- 已登录但未完善资料用户：可以继续浏览，也可以进入个人中心完善资料
- 已登录且已完善资料用户：可以发布评价；后续 V2 的点赞、回复等互动也要求资料完善

后端必须校验贡献型行为权限：

- 发布评价：需要登录 + 资料已完善
- 删除自己的评价：需要登录，且只能删除本人评价
- 点赞、回复：V2 功能，设计上需要登录 + 资料已完善
...
- 已完成：按校区查询 POI 列表
- 待完成：POI 详情接口 `GET /api/v1/pois/{poiId}`
...
- Spring Security 完整认证授权体系；当前仅使用 `spring-security-crypto` 提供 BCrypt 密码加密
- Elasticsearch
- RabbitMQ
- 微服务
- Kubernetes
...
`users` 表后续需要支持资料完善状态。实现方式可选择新增 `profile_completed` 字段，或基于 `identity_type` / `target_uni_id` / `current_uni_id` 动态判断；当前倾向是新增字段，降低后续权限判断复杂度。
